...
import eoriver.algorithms


def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.echo('Version ' + eoriver.__version__)
    ctx.exit()
...
def get_water_mask(region, output, filter_bounds, start, stop, scale, crs):
    ...
    # query water mask
    water_mask = eoriver.algorithms.get_water_mask(region, start, stop, scale, crs)
...
def get_network(region, output, filter_bounds, start, stop, scale, crs):
    ...
    network = eoriver.algorithms.get_water_network(region, start, stop, scale, crs)
...
def get_network_properties(region, output, filter_bounds, start, stop, scale, crs, step):
    ...
    features = eoriver.algorithms.get_water_network_properties(region, start, stop, scale, crs, step)
