...
class Network:
    ...
    def construct(self, spacing=[], skeletonFile=False, branchFile=False, degree=0.0):
        
        self._voronoi(opt="Qbb") #construct voronoi lines of voronoi polygons and keep only the ones inside them
        self._snap() #snap S-E points to real starting points
        if skeletonFile:
            self._save_skeleton(filename=str(skeletonFile)+".shp")
        self._clean() #remove all artifacts and keep only start-end point lines
        self._topology() #construct the branches with correct topology
        ...
        if branchFile:
            self._save_branches(filename=str(branchFile)+".shp")
        ...
        #find widths:
        #widths = self._get_widths(degree) #only with "experimental" below
        ...
        for gid in self.polys.keys():
            ...
            for branch in self.branches[gid].keys():
                ...
                for p in (self.branches[gid][branch]):
                    ...
                    #TODO: parallelize OR make R-tree structure to have faster search
                    for l in ls:
                        x0,y0 = p
                        x1,y1 = l[0]
                        x2,y2 = l[1]
...
#TODO: Make a general save function
    def _save_skeleton(self, filename="skeleton.shp"):
        print "\nWriting to file..."
...
#TODO: Make a general save function
    def _save_branches(self, filename="branches.shp"):
    ...
    def _lalasave(self, polygons, filename="voronoia.shp"):
    ...
    def get_chainage(self, branch=False):
        ...
        if not(branch):
            ...
            for j in range(1,len(x)):
                s = np.append(s,s[j-1] + np.sqrt((x[j]-x[j-1])**2+(y[j]-y[j-1])**2))
             
            return s #length of m.a. vertices from the start of the centerline
...
#            curvature = common.smooth(curvature,smoothdegree)
    ...
    def _read(self, filename, which = "poly"):
    ...
    def _overview2csv(self,csvdict):
